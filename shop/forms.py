...
class SubmitOrderForm(forms.Form):
    def __init__(self, *args, **kwargs):
        ...
        self.fields['number'] = forms.RegexField(regex=r'^\+?1?\d{9,15}$', required=True,label='Номер телефона',max_length=16)
        self.fields['email'] = forms.EmailField(max_length=64,required=True,label='e-mail')
